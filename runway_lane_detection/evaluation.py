...
from .utils.convert import str_to_points
...
class DetectionEvaluator:
    ...
    def _get_batch_labels(self, row: pd.Series) -> ImageDetections:
        ...
        for label_id, label_name in self._model.names_map.items():
            points_np = str_to_points(row[label_name])
        ...
        gt_dets = (
            np.stack(gt_dets) if len(gt_dets) else np.empty((0, 7))  # noqa: WPS507
        )
        return ImageDetections(gt_dets)

    def _get_batch(
        self,
        img_fpaths: list[Path],
        lbl_fpaths: list[Path],
    ...
    def evaluate(
        ...
        img_fpaths = [
            data_dpath / "images" / f"{fpath.stem}.PNG" for fpath in lbl_fpaths
        ]
